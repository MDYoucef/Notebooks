...
def create_kernel(hp):
    ...
    for i, c in enumerate(nidx):
        ...
        kernel += comp
    k = 4*len(nidx)
    binary = RBF(hp[k:k+len(bidx)], active_dim=bidx) * ConstantKernel(hp[k+len(bidx)], active_dim=bidx)
    k = k+len(bidx)
...
df = df.loc[(df['product_group'] == 'Product_25') & (df['customer_name'] == 'Customer_01')]
...
bidx = [X_train.columns.get_loc(i) for i in categorical+binary] #bidx columns index
...
lb, ub = 1e-5, 1e3
lb, ub = [lb, 0, lb, lb]*len(nidx) + [lb]*len(bidx)+[lb] + [lb] * 6, [ub, 3, ub, ub]*len(nidx) + [ub]*len(bidx)+[ub] + [ub] * 6 #kernel 1
...
#lb, ub = [lb]*len(nidx) + [lb, lb] + [lb]*len(cidx)+[lb] + [lb]*6, [ub]*len(nidx) + [lb, ub] + [ub]*len(cidx)+[ub] + [ub]*6
